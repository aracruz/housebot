...
const iptu = require('./iptuSearch');

const iptuService = async (contactUser, msg, contactNumber, nomeAtendente, client, chat) => {
   ...
   if (true) {
      delay(2000).then(async function () {
         ...
         delay(2000).then(async function () {
            const buttons_reply = new Buttons(houseMessage(`*ATENÇÃO:*\nO *IPTU 2023* ainda não está disponível, em caso de dúvida entre em contato com setor de cadastro imobiliário.\nPara pagar o *IPTU 2022* entre em contado com o setor de divida ativa.`), [{
               body: 'Voltar',
               id: 'reset_service'
            }, {
               body: 'Finalizar',
   ...
   if (contactUser.actual_level === 0) {
      ...
      delay(4000).then(async function () {
         const answer = `Para ver o IPTU 2022, é necessário informar o CPF/CNPJ ou a Inscrição municipal, qual opção deseja informar?`
      ...
      await ContactController.update({
         ...
         actual_level: 1,
         service: 'iptu_service'
      });
   }

   /*  Opçãoes para os serviços de IPTU*/
   else if (contactUser.service === 'iptu_service' && +contactUser.actual_level !== 0) {
      if (+contactUser.actual_level === 1) {
         ...
         if (tipoDocValido) {
            ...
            const imovelInscricoes = await (async () => {
               ...
               while (tatativasNavegar <= 10) {
                  const resultNavegation = await iptu.getByCpfOrCnpj(msg.body, tipoDocValido.toLowerCase())
         ...
         const pathPDF = `./store/boleto_iptu/${inscr_munic}_${busca_pcarcela}.pdf`
         await page.goto(`http://nfe.pma.es.gov.br:8081/services/lancamento_diversos_boleto.php?parcela=${busca_pcarcela}&inscricao=${busca_insc}&agrupamento=FEE&tipo_inscricao=FE&data_venc=${busca_data}&codigo_cnt=${busca_cnt}&tp=${busca_tp}`);
