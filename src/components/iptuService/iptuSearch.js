...
const getByCpfOrCnpj = (async (cpfOrCnpf, tipoDoc) => {
    ...
    try {
        await page.goto(`http://nfe.pma.es.gov.br:8081/services/redirect_auth.php?auth=$1$TeJ44FC/$8tel11.w4o1I.q2Af9YAE/&codigo=${cpfOrCnpf.replace(/[^0-9]/g, '')}&tpc=01&tipo_consulta=${tipoDoc}&pagina=lancamentos_diversos.php?cod=F`);
    ...
    const returnIptuInfo = await page.evaluate(async () => {
        ...
        if (imoveisNodeList.length === 0 && imoveisDadosIncricao.length === 0) {
            return {
                status: 200,
                description: 'Não existe parcela de IPTU lançada ou em aberto para o CPF/CNPJ informado',
                result: 0
            }
        }
