...
const cnega = require('./bciSearch');

const bciService = async (contactUser, msg, contactNumber, nomeAtendente, client, chat) => {
   ...
   if (true) {
      delay(2000).then(async function () {
         ...
         delay(2000).then(async function () {
            const buttons_reply = new Buttons(houseMessage(`*ATENÇÃO:*\nO *BCI 2023* ainda não está disponível, em caso de dúvida entre em contato com setor de cadastro imobiliário.`), [{
               body: 'Voltar',
               id: 'reset_service'
            }, {
               body: 'Finalizar',
